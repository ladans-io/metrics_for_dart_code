...
2. Change the plugin starter `metrics_for_dart_code` dependency in `tools\analyzer_plugin\pubspec.yaml` to a path dependency:

    ```yaml
    name: metrics_for_dart_code_plugin_loader
...
4. Run `dart pub get` in:
   - `metrics_for_dart_code` working copy
   - `tools\analyzer_plugin` directory in `metrics_for_dart_code` working copy
